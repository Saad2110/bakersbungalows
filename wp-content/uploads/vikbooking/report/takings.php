...
class VikBookingReportTakings extends VikBookingReport
{
	...
	public function __construct()
	{
		...
		$this->reportName = JText::_('VBO_TAKINGS');
		$this->reportFilters = [];
	...
	public function getFilters()
	{
		...
		$filter_opt = array(
			'label' => '<label for="type">' . JText::_('VBSTATUS') . '</label>',
			'html' => $types_sel_html,
			'type' => 'select',
			'name' => 'type'
		);
	...
	public function getReportData()
	{
		...
		$this->cols = array(
			...
			array(
				...
				'sortable' => 1,
				'label' => JText::_('VBPVIEWORDERSTHREE')
			),
			// event
			array(
				'key' => 'event',
				...
				'sortable' => 1,
				'label' => JText::_('VBOBOOKHISTORYLBLTYPE')
			),
			// amount paid
			array(
				'key' => 'amount_paid',
		...
		$def_pay_meth = JText::_('VBOREPORTTOPCUNKNC');
			...
			array_push($this->rows, array(
				array(
					...
					'callback' => function ($val) use ($df, $datesep) {
						return JHtml::_('date', $val, str_replace("/", $datesep, $df) . ' H:i:s');
					},
		...
		$this->footerRow[] = [
			[
				...
				'value' => '<h3>' . JText::_('VBO_TOTAL_TAKINGS') . '</h3>',
			],
			[
				'value' => '',
			],
