...
class VikCronJob
{
	...
	public static function getAdminParameters()
	{
		...
		return array(
			'cron_lbl' => array(
				'type' => 'custom',
				'label' => '',
				'html' => '<h4><i class="vboicn-bubble"></i><i class="vboicn-alarm"></i>SMS Reminder</h4>'
			),
			'checktype' => array(
				'type' => 'select',
			...
			'remindbefored' => array(
				...
				'default' => '2', 'attributes' => array('style' => 'width: 60px !important;')
			),
			'test' => array(
				'type' => 'select',
				'label' => JText::_('VBOCRONSMSREMPARAMTEST').'//'.JText::_('VBOCRONSMSREMPARAMTESTHELP'),
			...
			'tpl_text' => array(
				...
				'attributes' => array('id' => 'tpl_text', 'style' => 'width: 70%; height: 130px;')
			),
			'buttons' => array(
				'type' => 'custom',
				'label' => '',
	...
	public function run()
	{
		$dbo = JFactory::getDbo();
		$this->getCronData();
		$start_ts = $this->use_date = mktime(0, 0, 0, date('n'), ($this->checktype == 'checkout' ? ((int)date('j') - $this->params['remindbefored']) : ((int)date('j') + $this->params['remindbefored'])), date('Y'));
		$end_ts = mktime(23, 59, 59, date('n'), ($this->checktype == 'checkout' ? ((int)date('j') - $this->params['remindbefored']) : ((int)date('j') + $this->params['remindbefored'])), date('Y'));
		if ($this->debug) {
	...
	// this method is called after the cron has been executed
